...
func TestDSCustomPrinter_PrintNonJSON(t *testing.T) {
	raw := "hello world"
	buf := &bytes.Buffer{}
	printer := NewDSCustomPrinter(buf)
	entry := &Entry{
		Raw: []byte(raw),
	}
	printer.Print(entry)
...
func TestDSCustomPrinter_Print(t *testing.T) {
	tests := []struct {
		name      string
		json      string
		formatted string
...
func TestDSCustomPrinter_PrintWithColors(t *testing.T) {
	var logs = []string{
